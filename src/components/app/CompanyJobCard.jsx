      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200 h-full flex flex-col">
        <div className="flex-grow">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-3">
            ...
            <span>Publié le : {job.date_publication}</span>
          </div>
...
const CompanyJobCard = ({ jobs }) => {
  ...
  const JOBS_PER_PAGE = 6; // On peut afficher 6 offres par page par exemple

  // Réinitialiser la page si la liste des jobs change
  useEffect(() => {
    setCurrentPage(1);
  }, [jobs]);

  if (!jobs || jobs.length === 0) {
    return <p className="text-center text-gray-500 py-10">Cette entreprise n'a aucune offre d'emploi pour le moment.</p>;
  }
