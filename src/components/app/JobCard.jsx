  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        console.error(t('jobOffers.errorLoad'), err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <Link to={`/jobOffers/${job.id}`}>
        <div className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
            ...
            <div className="flex flex-col gap-3">
              <div className='flex'>
                <div className="w-25 h-25 bg-gray-100 rounded-lg flex items-center justify-center self-start">
                  <img src={`/storage/public/${job.employeur.logo}`} alt={t('jobOffers.companyLogoAlt', { company: job.employeur?.nom_entreprise })} className="w-16 h-16 sm:w-20 sm:h-20 object-contain" />
                </div>
                <div className='mt-7 ml-3 font-semibold text-xl'>{job.employeur.nom_entreprise || t('jobOffers.unknownCompany')}</div>
              </div>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-3">
            ...
            <span>{t('jobOffers.publicationDate')} {job.date_publication}</span>
          </div>

          <div className="flex items-center gap-2 text-sm text-gray-600 mb-3">
            <MapPin className="w-4 h-4" />
            <span>{t('jobOffers.location')} {job.ville}, {job.pays}</span>
          </div>
          ...
          <div className="space-y-2 mb-4">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <Clock className="w-4 h-4" />
              <span>{job.workType || t('jobOffers.notSpecified')}</span>
            </div>
          </div>
        </div>
      </Link>
        ...
        <div className="bg-white border-b border-gray-200">
          <div className="mx-auto px-10 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between py-6">
              <div className="flex items-center">
                <h1 className="text-2xl font-semibold text-emerald-400">{t('jobOffers.availableJobs')}</h1>
              </div>
            </div>
          </div>
          ...
          <div className="mx-auto px-10 sm:px-6 lg:px-8 py-8">
            {loading ? (
              ...
              <p className="text-center text-gray-500">{t('jobOffers.noJobsFound')}</p>
            ) : (
              <>
                <AnimatePresence>
                ...
                {paginationEnabled && totalPages > 1 && (
                  <div className="flex justify-center items-center space-x-4 mt-8">
                    ...
                    <span className="text-gray-700 font-medium">
                      {t('jobOffers.page', { current: currentPage, total: totalPages })}
                    </span>
                    <button
