...
const JobCard = ({ searchTerm, locationTerm, selectedContract, selectedEducation, paginationEnabled = false, limit = 6 }) => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        setJobs(response.data); // selon ton API : ajuste si c'est response.data.data
      } catch (err) {
  ...
  const totalPages = paginationEnabled && filteredJobs ? Math.ceil(filteredJobs.length / JOBS_PER_PAGE) : 0;
            ...
            <div className="flex flex-col gap-3">
              <div className='flex'>
                <div className="w-25 h-25 bg-gray-100 rounded-lg flex items-center justify-center self-start">
                  <img src={`${job.employeur.logo}`} alt="Bantulink Logo" className="w-16 h-16 sm:w-20 sm:h-20 object-contain" />
                </div>
        ...
        <div className="mx-auto px-10 sm:px-6 lg:px-8 py-8">
          {loading ? (
            ...
            <>
              ...
              {!paginationEnabled && filteredJobs && filteredJobs.length > limit && (
