...
const AutresRessources = () => {
  ...
  const validationSchema = Yup.object({
    portfolio_link: Yup.string().url(t('resources.invalidUrl')).nullable(),
    linkedin_link: Yup.string().url(t('resources.invalidUrl')).nullable(),
    github_link: Yup.string().url(t('resources.invalidUrl')).nullable(),
    behance_link: Yup.string().url(t('resources.invalidUrl')).nullable(),
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        await api.put('/profile/particulier', values);
        toast.success(t('resources.successUpdate'));
        setIsModalOpen(false);
      } catch (error) {
        console.error("Update error:", error.response?.data || error.message);
        toast.error(t('resources.errorUpdate'));
      } finally {
        setSubmitting(false);
      }
    },
    ...
    <div className="bg-white p-6 rounded-lg shadow-md max-w-[95%] mx-auto my-8 border border-gray-200">
      ...
      >
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-400">
          <h2 className="text-xl font-semibold text-blue-800">{t('resources.title')}</h2>
            ...
            <DialogContent className="sm:max-w-lg bg-white rounded-lg shadow-md p-0">
              <DialogHeader className="pb-4 border-b border-gray-200">
                <DialogTitle className="text-xl font-semibold text-gray-800 pt-6 px-6">
                  {t('resources.modalTitle')}
                </DialogTitle>
              </DialogHeader>
              <form onSubmit={formik.handleSubmit} className="p-6 space-y-4">
                <div>
                  <label htmlFor="portfolio_link" className="block text-gray-700 font-medium mb-2">
                    {t('resources.portfolio')}
                  </label>
                  <input
                    id="portfolio_link"
                    type="text"
                    ...
                    placeholder={t('resources.portfolioPlaceholder')}
                  />
                ...
                <div>
                  <label htmlFor="linkedin_link" className="block text-gray-700 font-medium mb-2">
                    {t('resources.linkedin')}
                  </label>
                  <input
                    id="linkedin_link"
                    type="text"
                    ...
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder={t('resources.linkedinPlaceholder')}
                  />
                ...
                <div>
                  <label htmlFor="behance_link" className="block text-gray-700 font-medium mb-2">
                    {t('resources.behance')}
                  </label>
                  <input
                    id="behance_link"
                    ...
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder={t('resources.behancePlaceholder')}
                  />
                ...
                <div>
                  <label htmlFor="github_link" className="block text-gray-700 font-medium mb-2">
                    {t('resources.github')}
                  </label>
                  <input
                    id="github_link"
                    ...
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder={t('resources.githubPlaceholder')}
                  />
        ...
        <div className="space-y-4 text-sm text-gray-700">
          <div className="flex items-center gap-4">
            <p className="w-40 text-gray-700 font-medium">{t('resources.portfolio')}</p>
            {renderLink(particulier?.portfolio_link, Globe, 'Portfolio') || <span className="text-gray-500">{t('resources.notProvided')}</span>}
          </div>
          <div className="flex items-center gap-4">
            <p className="w-40 text-gray-700 font-medium">{t('resources.linkedin')}</p>
            {renderLink(particulier?.linkedin_link, Linkedin, 'LinkedIn') || <span className="text-gray-500">{t('resources.notProvided')}</span>}
          </div>
          <div className="flex items-center gap-4">
            <p className="w-40 text-gray-700 font-medium">{t('resources.behance')}</p>
            {renderLink(particulier?.behance_link, Globe, 'Behance') || <span className="text-gray-500">{t('resources.notProvided')}</span>}
          </div>
          <div className="flex items-center gap-4">
            <p className="w-40 text-gray-700 font-medium">{t('resources.github')}</p>
            {renderLink(particulier?.github_link, Github, 'GitHub') || <span className="text-gray-500">{t('resources.notProvided')}</span>}
          </div>
        </div>
      </motion.section>
    </div>
