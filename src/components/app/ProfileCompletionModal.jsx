...
const ProfileCompletionModal = ({ isOpen, onClose, onComplete }) => {
  ...
  const validationSchema = Yup.object({
    date_naissance: Yup.date().required(t('profileModal.birthDateRequired')),
    telephone: Yup.string().required(t('profileModal.phoneRequired')),
    adresse: Yup.string().required(t('profileModal.addressRequired')),
    ville: Yup.string().required(t('profileModal.cityRequired')),
    pays: Yup.string().required(t('profileModal.countryRequired')),
    titre_professionnel: Yup.string().required(t('profileModal.jobTitleRequired')),
    resume_profil: Yup.string().required(t('profileModal.profileSummaryRequired')),
    cv_link: Yup.string().url(t('profileModal.invalidCVUrl')),
    lettre_motivation_link: Yup.string().url(t('profileModal.invalidLetterUrl')),
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        toast.success(t('profileModal.profileUpdated'));
        onClose();
        if (onComplete) {
          onComplete();
        }
        ...
        toast.error(t('profileModal.updateError'));
      } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('profileModal.completeProfile')}</DialogTitle>
          <DialogDescription>
            {t('profileModal.completeProfileDesc')}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={formik.handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.birthDate')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.phone')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.address')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.city')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.country')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.jobTitle')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.profileSummary')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.profileImage')}</label>
            ...
            {previewImage && (
              <div className="mt-2">
                <img src={previewImage} alt={t('profileModal.preview')} className="h-20 w-20 rounded-full object-cover border-2 border-gray-300" />
              </div>
            )}
          </div>
          
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.cv')}</label>
            ...
            {particulier?.cv_link && !cvFile && (
              <p className="text-gray-500 text-xs mt-1">{t('profileModal.currentCV')} <a href={`${particulier.cv_link}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">{t('profileModal.view')}</a>. {t('profileModal.uploadNewReplace')}</p>
            )}
          </div>
          
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('profileModal.motivationLetter')}</label>
            ...
            {particulier?.lettre_motivation_link && !lettreFile && (
              <p className="text-gray-500 text-xs mt-1">{t('profileModal.currentLetter')} <a href={`${particulier.lettre_motivation_link}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">{t('profileModal.view')}</a>. {t('profileModal.uploadNewReplace')}</p>
            )}
          </div>
          ...
          <div>
            <label className="flex items-center space-x-2">
              ...
              <span className="text-sm font-medium text-gray-700">{t('profileModal.profileVisibleRecruiters')}</span>
            </label>
          </div>
